(function () {
  ...
  window.addEventListener("mousedown", function (event) {
    ...
    if (isTabGroup.success) {
      ...
      if (
        isTabItem.success &&
        !hasParentClass(event.target, "icon-close-tab").success
      ) {
      ...
      if (currTabGroup.activeDraggingItem) {
        ...
        var newPos = (function () {
          ...
          if (pos === null) {
            ...
            pos =
              Math.abs(x - firstItem.bounding.left) >=
              Math.abs(x - (lastItem.bounding.left + lastItem.bounding.width))
                ? lastItem.nodePos
                : firstItem.nodePos;
          }
          return pos;
        ...
        diffX += diffX >= 0 ? -2 : 2;
        if (item.nodePos != newPos)
          item.style.transform = "translate(" + diffX + "px, 0px)";
        setTimeout(
          function (group) {
          ...
          if (i < item.nodePos) {
            if (x < rightX && isDraggable(tabItem)) {
              tabItem.style.transform =
                "translate(" + bounding.width + "px, 0px)";
  ...
  HTMLDivElement.prototype.addTab = function (options) {
    ...
    if (isTabGroup.success) {
      ...
      var opts = options.fillDefaults({
        ...
        isActive: true,
        animated: true,
        content: document.createTextNode("Untitle Request"),
      });
      var items = group.getElementsByClassName("tab-item");
      var itemsDraggable = getItemsDraggable(items);
      ...
      newTab.appendChild(
        typeof opts.content === "string"
          ? document.createTextNode(opts.content)
          : opts.content
      );
      if (itemsDraggable.length > 0) {
        var appendRefItem =
          opts.position === "first"
            ? items[itemsDraggable[0].nodePos]
            : items[itemsDraggable[itemsDraggable.length - 1].nodePos + 1];
      } else {
      ...
      if (item.classList.contains("active")) {
        ...
        group.activateItem(
          newActiveItem != undefined
            ? isEnableable(newActiveItem)
              ? newActiveItem
              : items[item.nodePos - 1]
            : itemsDraggable[0]
        );
      }
  ...
  Object.prototype.fillDefaults = function (defaults) {
    /* Just a small self-made function to fill default objects */
    var keys = Object.keys(defaults);
    for (var i = 0; i < keys.length; i++) {
      if (typeof defaults[keys[i]] == "object")
        this[keys[i]] =
          this[keys[i]] != undefined
            ? this[keys[i]].fillDefaults(defaults[keys[i]])
            : defaults[keys[i]];
